...
export class PlanMantenimientoIndustrialesService {
  ...
  async getPlanesByPeriodo(ano: number, mes: number, tecnicoId?: number): Promise<any[]> {
    const url = tecnicoId ? `${this.baseUrl}/planes/periodo/${ano}/${mes}?tecnicoId=${tecnicoId}` : `${this.baseUrl}/planes/periodo/${ano}/${mes}`;
    return firstValueFrom(
      this.httpClient.get<any[]>(url, createHeaders())
    );
  }

  // Obtener planes de mantenimiento por rango de meses
  async getPlanesByRango(ano: number, mesInicio: number, mesFin: number, tecnicoId?: number): Promise<any[]> {
    const url = tecnicoId ? `${this.baseUrl}/planes/rango/${ano}/${mesInicio}/${mesFin}?tecnicoId=${tecnicoId}` : `${this.baseUrl}/planes/rango/${ano}/${mesInicio}/${mesFin}`;
    return firstValueFrom(
      this.httpClient.get<any[]>(url, createHeaders())
    );
  }
