...
export class EquiposIndustrialesService {
  ...
  // Dar de baja un equipo con reporte detallado
  async darDeBajaEquipo(idEquipo: number, datosBaja: any): Promise<any> {
    return firstValueFrom(
      this.httpClient.put<any>(`${this.baseUrl}/bajaequipo/${idEquipo}`, datosBaja, createHeaders())
    );
  }

  // Obtener reporte de baja de un equipo
  async getReporteBaja(idEquipo: number): Promise<any> {
    return firstValueFrom(
      this.httpClient.get<any>(`${this.baseUrl}/baja/equipo/${idEquipo}`, createHeaders())
    );
  }
