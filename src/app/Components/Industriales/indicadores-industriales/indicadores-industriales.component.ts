...
import { IndicadoresIndustrialesService } from '../../../Services/appServices/industrialesServices/indicadores/indicadores-industriales.service';
...
interface IEquipo { id: number; nombres?: string; nombre?: string; codigo?: string; marca?: string; serie?: string; placa?: string; }
interface IServicio { id: number; nombres?: string; nombre?: string; }
...
@Component({
    selector: 'app-indicadores-industriales',
    standalone: true,
    imports: [
        CommonModule,
        FormsModule,
...
export class IndicadoresIndustrialesComponent {
    ...
    private indicadoresSrv = inject(IndicadoresIndustrialesService);
    ...
    async refrescar() {
        ...
        try {
            const [reportesData, indicadores] = await Promise.all([
                this.indicadoresSrv.getReportesPorRango(inicioStr, finStr),
                this.indicadoresSrv.getIndicadoresCumplimiento(inicioStr, finStr)
            ]);

            this.reportes.set(Array.isArray(reportesData) ? reportesData : []);
            this.indicadoresData.set(indicadores);
        } catch (e) {
            console.error(e);
            this.reportes.set([]);
        } finally {
        ...
        return {
            ...
            plugins: {
                ...
                tooltip: {
                    ...
                    backgroundColor: 'rgba(255, 255, 255, 0.9)', titleColor: '#1f2937', bodyColor: '#4b5563',
                    borderColor: '#e5e7eb', borderWidth: 1, padding: 10, boxPadding: 4
                }
            },
            scales: {
                x: { ticks: { autoSkip: true, font: { size: 12, family: "'Inter', sans-serif" }, color: '#6b7280' }, grid: { display: false } },
                y: { beginAtZero: true, ticks: { precision: 0, font: { size: 12, family: "'Inter', sans-serif" }, color: '#6b7280' }, grid: { color: '#f3f4f6', borderDash: [5, 5] } }
            },
    ...
    getResponsableOptions() {
        return {
            ...
            scales: {
                ...
                y: { beginAtZero: true, suggestedMax: 120, ticks: { stepSize: 30, precision: 0, font: { size: 12 }, color: '#6b7280' }, grid: { color: '#f3f4f6' } }
            }
        } as any;
    }
    ...
    // --- Computed para Indicadores Backend ---

    preventivoMeterData = computed(() => {
        const d = this.indicadoresData();
        if (!d || !d.preventivo) return [];
        const p = d.preventivo;
        const total = p.programados || 1;
        const completadosPct = Math.round((p.completados / total) * 100);
        const realizadosPct = Math.round(((p.realizados - p.completados) / total) * 100);
        const pendientesPct = 100 - completadosPct - realizadosPct;
        return [
            { label: 'Programados', value: pendientesPct, color: '#94a3b8', icon: 'pi pi-calendar' },
            { label: 'Realizados', value: realizadosPct, color: '#f59e0b', icon: 'pi pi-clock' },
            { label: 'Completados', value: completadosPct, color: '#10b981', icon: 'pi pi-check-circle' }
        ];
    });
    ...
    duracionStats = computed(() => {
        ...
        const calcProm = (tipo: string) => {
            const filtrados = rs.filter(r => r.tipoMantenimiento === tipo);
            let suma = 0, count = 0;
            filtrados.forEach(r => {
                const dur = this.calcularDuracionReporte(r);
                if (dur && dur > 0) { suma += dur; count++; }
            });
            if (count === 0) return '00:00:00';
            const prom = suma / count;
            const h = Math.floor(prom / 60);
            const m = Math.floor(prom % 60);
            const s = Math.floor((prom * 60) % 60);
            return `${String(h).padStart(2, '0')}:${String(m).padStart(2, '0')}:${String(s).padStart(2, '0')}`;
        };

        return {
            preventivo: calcProm('Preventivo'),
            correctivo: calcProm('Correctivo')
        };
    });

    preventivoInfo = computed(() => {
        const d = this.indicadoresData();
        return d?.preventivo || { programados: 0, realizados: 0, completados: 0, cumplimiento: 0 };
    });
    ...
    porUsuarioChartData = computed(() => {
        ...
        rs.forEach(r => {
            ...
            if (r.tipoMantenimiento === 'Preventivo') current.preventivo++;
            else if (r.tipoMantenimiento === 'Correctivo') current.correctivo++;
        ...
        return {
            labels,
            datasets: [
                { label: 'Preventivo', data: dataPrev, backgroundColor: '#3b82f6', borderColor: '#2563eb', borderWidth: 1, borderRadius: 4, barPercentage: 0.7 },
                { label: 'Correctivo', data: dataCorr, backgroundColor: '#f97316', borderColor: '#ea580c', borderWidth: 1, borderRadius: 4, barPercentage: 0.7 }
            ]
        };
    });
    ...
    porServicioChartData = computed(() => {
        ...
        const mapa = this.groupCount(rs, r => (r.servicio as any)?.nombres || (r.servicio as any)?.nombre || 'Sin servicio');
        ...
        return {
            labels,
            datasets: [{
                label: 'Top 5 Correctivos por Servicio',
                data,
    ...
    porFallaChartData = computed(() => {
        ...
        return {
            ...
            datasets: [{
                ...
                data: orden.map(k => mapa.get(k) ?? 0),
                backgroundColor: this.bgColors, borderColor: this.borderColors,
                borderWidth: 1, borderRadius: 8, barPercentage: 0.6
            }]
        };
    });
    ...
    porMesChartData = computed(() => {
        ...
        return {
            ...
            datasets: [{
                ...
                pointBackgroundColor: '#ffffff', pointBorderColor: '#3b82f6', pointBorderWidth: 2, tension: 0.4
            }]
        };
    });
    ...
    topEquiposChartData = computed(() => {
        const rs = this.reportes();
        const mapa = this.groupCount(rs, r => r.equipo?.nombres || r.equipo?.nombre || `Equipo ${r.equipoIndustrialIdFk}`);
    ...
    duracionPromedioPorTipoChartData = computed(() => {
        ...
        const avg = this.groupAvg(
            ...
            r => this.calcularDuracionReporte(r)
        );
        const orden: TipoMantenimiento[] = ['Preventivo', 'Correctivo', 'Predictivo', 'Otro'];
        ...
        return {
            ...
            datasets: [{
                label: 'Minutos (prom.)',
                data,
                backgroundColor: 'rgba(99, 102, 241, 0.7)', borderColor: '#4f46e5',
                borderWidth: 1, borderRadius: 8, barPercentage: 0.6
            }]
        };
    });

    // Preventivos pendientes por responsable
    pendientesPorResponsableChartData = computed(() => {
        ...
        return {
            ...
            datasets: [{
                ...
                data,
                backgroundColor: '#f59e0b', borderColor: '#d97706', borderWidth: 1
            }]
        };
    });
    ...
    private calcularDuracionReporte(r: Reporte): number | null {
        // 1. Intentar con horaTotal
        if (r.horaTotal) {
            const m = /^(\d{1,2}):(\d{2})(?::(\d{2}))?$/.exec(r.horaTotal);
            if (m) return +m[1] * 60 + +m[2] + (+(m[3] ?? 0)) / 60;
        }
        // 2. Fallback: calcular desde horaInicio y horaTerminacion
        if (!r.horaInicio || !r.horaTerminacion) return null;
        const fechaI = r.fechaRealizado || '2000-01-01';
        const fechaF = r.fechaFin || fechaI;
        const inicio = new Date(`${fechaI}T${r.horaInicio}`);
        const fin = new Date(`${fechaF}T${r.horaTerminacion}`);
