...
export class CrearReporteIndustrialComponent implements OnInit {
  ...
  initForm() {
    ...
    if (this.tipoMantenimiento === 'Preventivo') {
      ...
      if (motivoFallo) {
        // Remove HTML tags that might come from the rich text editor in mesa de servicios
        const cleanMotivo = motivoFallo.replace(/<[^>]*>?/gm, '');
        this.reporteForm.get('motivo')?.setValue(cleanMotivo);
      }
    }
  ...
  calcularHoras() {
    ...
    if (fechaInicio && horaInicio && fechaFin && horaFin) {
      try {
        ...
        if (partsInicio.length >= 2) {
          inicio.setHours(Number(partsInicio[0]), Number(partsInicio[1]));
        }

        const fin = new Date(fechaFin);
        const partsFin = horaFin.split(':');
        if (partsFin.length >= 2) {
          fin.setHours(Number(partsFin[0]), Number(partsFin[1]));
        }
  ...
  async onSubmit() {
    if (this.reporteForm.invalid) {
      ...
      // Iterate to find invalid fields for better UX debugging
      Swal.fire('Error', 'Debe llenar obligatoriamente todos los espacios del formulario.', 'warning');
      return;
    }
    ...
    try {
      ...
      try {
        ...
        if (casoIdPendiente) {
          ...
          const mensajeCierre = sessionStorage.getItem('MotivoFalloIndustrial') || 'Reporte industrial finalizado';
          ...
          closeData.append('usuarioId', userId.toString());
          closeData.append('mensajeFinal', mensajeCierre);
          await this.mesaService.closeCaso(Number(casoIdPendiente), closeData).toPromise();
        }
      } catch (err) {
        console.error('Error vinculando o cerrando reporte a Mesa de Servicios', err);
      }
