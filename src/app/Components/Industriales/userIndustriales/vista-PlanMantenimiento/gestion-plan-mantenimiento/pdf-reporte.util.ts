...
export async function generarReporteMantenimientoPDF(reporte: any, equipo: any, rutina: any[] = []) {
    const doc = new jsPDF({ orientation: 'portrait', unit: 'mm', format: 'letter' });

    // Load Logo
    let logoImg: string | null = null;
    try {
        const response = await fetch('/LogoReportes.png');
        if (response.ok) {
        ...
        console.error('Could not load logo in pdf-reporte.util', e);
    }
    ...
    drawCell('CÓDIGO: GE-F-XX\nVERSIÓN: 01', startX, Y, col1W, 13, { align: 'center', fontSize: 6 });
    ...
    drawCell('', startX + col1W + centerW, Y, col3W, 13, { align: 'center' }); // Empty white square for logo
    if (logoImg) {
        try {
            doc.addImage(logoImg, 'PNG', startX + col1W + centerW + 2.5, Y + 1.5, col3W - 5, 10);
        } catch (e) {
            console.error('Error drawing logo in pdf-reporte.util', e);
        }
    }
    Y += 15;
