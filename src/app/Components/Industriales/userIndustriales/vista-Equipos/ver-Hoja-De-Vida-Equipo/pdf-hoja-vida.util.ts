...
export function generarHojaVidaPDF(equipoInfo: any, hojaVida: any, datosTecnicos: any, proveedor: any, registroApoyo: any) {
    ...
    dc(v(equipoInfo?.responsableInd?.nombres), startX + 33, Y, 72, rowH, { fontSize: 5 });
    dc('ESTADO:', startX + 105, Y, 25, rowH, { bold: true, fontSize: 5 });
    dc(!equipoInfo?.estadoBaja ? 'ACTIVO' : 'DE BAJA', startX + 130, Y, dataW - 130, rowH, { fontSize: 5 });
    ...
    const techRows = [
        ...
        ['FRECUENCIA:', v(dt?.frecuencia), 'Hz', 'OTROS:', v(dt?.otros), '', 'AIRE:', chk(dt?.aire), '', ''],
    ...
    // ── CLASE DE TECNOLOGÍA | PERIODICIDAD | MANTENIMIENTO ──
    const ctW = 84;
    const pmW = 39;
    const maW = W - ctW - pmW;
    const hdr2H = 5.5;
    ...
    dc('PERIODICIDAD DEL\nMANTENIMIENTO:', startX + ctW, Y, pmW, hdr2H, { bold: true, fontSize: 4, align: 'center' });
    dc('MANTENIMIENTO\nACTUAL:', startX + ctW + pmW, Y, maW, hdr2H, { bold: true, fontSize: 5, align: 'center' });
    Y += hdr2H;
    ...
    const classRows = [
        { ctL: 'ELÉCTRICO:', ctV: clase === 'Electrico', ctL2: 'ELECTRÓNICO:', ctV2: clase === 'Electronico', pmL: 'TRIMESTRAL:', pmV: perM === 3, maL: 'PROPIO:', maV: mant === 'Propio' },
        { ctL: 'MECÁNICO:', ctV: clase === 'Mecanico', ctL2: 'ELECTROMECÁN:', ctV2: clase === 'Electromecanico', pmL: 'CUATRIMES:', pmV: perM === 4, maL: 'CONTRATADO:', maV: mant === 'Contratado' },
        { ctL: 'HIDRÁULICO:', ctV: clase === 'Hidraulico', ctL2: 'NEUMÁTICO:', ctV2: clase === 'Neumatico', pmL: 'SEMESTRAL:', pmV: perM === 6, maL: 'COMODATO:', maV: mant === 'Comodato' },
        { ctL: 'VAPOR:', ctV: clase === 'Vapor', ctL2: 'SOLAR:', ctV2: clase === 'Solar', pmL: 'ANUAL:', pmV: perM === 12, maL: 'GARANTÍA:', maV: mant === 'Garantia' }
    ];

    classRows.forEach(r => {
        dc(r.ctL, startX, Y, 25, rowH, { fontSize: 4 });
        dc(chk(r.ctV), startX + 25, Y, 7, rowH, { align: 'center', fontSize: 5 });
        dc(r.ctL2, startX + 32, Y, 35, rowH, { fontSize: 4 });
        dc(chk(r.ctV2), startX + 67, Y, 10, rowH, { align: 'center', fontSize: 5 });
        dc('', startX + 77, Y, 7, rowH); // padding
        dc(r.pmL, startX + ctW, Y, 28, rowH, { fontSize: 4 });
        dc(chk(r.pmV), startX + ctW + 28, Y, 11, rowH, { align: 'center', fontSize: 5 });
        dc(r.maL, startX + ctW + pmW, Y, 32, rowH, { fontSize: 5 });
        dc(chk(r.maV), startX + ctW + pmW + 32, Y, maW - 32, rowH, { align: 'center', fontSize: 5 });
        Y += rowH;
    });
