...
export class VerHojaDeVidaEquipoComponent implements OnInit {
    ...
    loadUserRole() {
        ...
        if (token) {
            ...
            if (rawRole === '8' || rawRole === 'ADMINMANTENIMIENTO' || rawRole === 'USERMANTENIMIENTO' || rawRole === '1' || rawRole === 'SUPERADMIN' || rawRole === '2' || rawRole === 'SYSTEMADMIN') {
                this.userRole = 'ADMINMANTENIMIENTO';
            } else if (rawRole === '9' || rawRole === 'TECNICOMANTENIMIENTO' || rawRole === '7' || rawRole === 'BIOMEDICATECNICO') {
                this.userRole = 'TECNICOMANTENIMIENTO';
            } else if (rawRole === '10' || rawRole === 'USERMANTENIMIENTO' || rawRole === '5' || rawRole === 'SYSTEMUSER' || rawRole === '6' || rawRole === 'BIOMEDICAUSER') {
    ...
    async cargarDatos() {
        try {
            ...
            // Construir URL de imagen del equipo (getImageUrl se encarga de la foto específica o el tipo)
            this.imagenUrl = this.getImageUrl(this.hojaVida?.foto);
    ...
    descargarDocumento(id: number, filename: string) {
    ...
    getImageUrl(fotoPath: string): string {
        if (fotoPath) {
            const filename = fotoPath.split('\\').pop()?.split('/').pop();
            if (filename) {
                return `${API_URL}/uploads/industriales/Hoja De Vida/${filename}`;
            }
        }
        
        if (this.equipoInfo && this.equipoInfo.tipoEquipoInd && this.equipoInfo.tipoEquipoInd.nombres) {
            const tipo = this.equipoInfo.tipoEquipoInd.nombres;
            return `${API_URL}/imagenequipotipo/${encodeURIComponent(tipo)}`;
        }
