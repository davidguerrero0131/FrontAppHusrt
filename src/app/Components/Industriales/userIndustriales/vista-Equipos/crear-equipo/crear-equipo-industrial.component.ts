...
export class CrearEquipoIndustrialComponent implements OnInit {
  ...
  constructor(
    ...
    this.equipoForm = this.formBuilder.group({
      ...
      estadoBaja: [false],
      estado: ['Bueno'],
      tipoEquipoIdFk: ['', Validators.required],
      servicioIdFk: ['', Validators.required],
      sedeIdFk: ['', Validators.required],
      responsableIdFk: ['', Validators.required]
    });
  }
  ...
  async guardar() {
    if (this.equipoForm.valid) {
      this.equipo = {
        ...
        registroSanitario: this.equipoForm.get('registroInvima')?.value,
  ...
  async iniciarFechasMantenimiento() {
    ...
    if (frecuencia > 0) {
      ...
      // Si periodicidad es 0, guardar directamente
  ...
  validarFechasMantenimiento() {
    ...
    // Guardar el equipo con todas las fechas
    this.guardarEquipoConFechas();
  }
