...
const codeToAdd = 
  ...
  intervencionOptions = [
    { name: '1 vez al a\\u00f1o (Anual)', value: 1 },
    { name: '2 veces al a\\u00f1o (Semestral)', value: 2 },
    { name: '3 veces al a\\u00f1o (Cuatrimestral)', value: 3 },
    { name: '4 veces al a\\u00f1o (Trimestral)', value: 4 }
  ];
  ...
  tipoActividadGlobal: string = 'Calibraci\\u00f3n';

  tipoActividadOptions: any[] = [
    { label: 'Calibraci\\u00f3n', value: 'Calibraci\\u00f3n' },
    { label: 'Calificaci\\u00f3n', value: 'Calificaci\\u00f3n' },
    { label: 'Mantenimiento Correctivo', value: 'Mantenimiento Correctivo' },
    { label: 'Inspecci\\u00f3n', value: 'Inspecci\\u00f3n' }
  ];
  ...
  initOpcionesHV() {
    this.opcionesHV = [
      ...
      {
        label: 'Registrar Actividad Metrol\\u00f3gica',
  ...
  updateCalculatedText() {
    ...
    if (this.selectedPlans.length === 0) {
      this.calculatedMonthsText = 'Seleccione intervenciones v\\u00e1lidas.';
      return;
    }
  ...
  async savePlan() {
    try {
      if (this.libreMantenimiento) {
        ...
        Swal.fire('Atenci\\u00f3n', 'Calcule el ciclo de mantenimiento antes de guardar.', 'warning');
        return;
      }
      const dataToSave = {
  ...
  openPlanMetrologiaDialog(equipo: any) {
    ...
    if (equipo.planesActividadMetrologica && equipo.planesActividadMetrologica.length > 0) {
      ...
      this.tipoActividadGlobal = firstPlan.tipoActividad || 'Calibraci\\u00f3n';
      ...
      this.tipoActividadGlobal = 'Calibraci\\u00f3n';
  ...
  updateCalculatedTextMetrologia() {
    if (this.libreActividadesMetrologicas) {
      this.calculatedMonthsTextMetrologia = 'El equipo no requiere actividades metrol\\u00f3gicas programadas.';
      return;
    }
    if (this.selectedPlansMetrologia.length === 0) {
      this.calculatedMonthsTextMetrologia = 'Seleccione intervenciones v\\u00e1lidas.';
      return;
    }
  ...
  async savePlanMetrologia() {
    try {
      if (this.libreActividadesMetrologicas) {
        ...
        Swal.fire('Atenci\\u00f3n', 'Calcule el ciclo de metrolog\\u00eda antes de guardar.', 'warning');
        return;
      }
      const dataToSave = {
        equipoId: this.currentEquipo.id,
      ...
      await this.equipoService.updatePlanMantenimiento(this.currentEquipo.id, dataToSave);
      Swal.fire('\\u00c9xito', 'Plan de metrolog\\u00eda actualizado.', 'success');
  ...
  async registrarMetrologia() {
    ...
    try {
      ...
      Swal.fire('\\u00c9xito', 'Actividad metrol\\u00f3gica registrada exitosamente', 'success');
...
html = html.replace(
  ...
  '<p-button label="Ver Hoja de Vida" icon="pi pi-file" (onClick)="verHojaVida()" styleClass="mr-2"></p-button>\n    <p-splitbutton label="Opciones" icon="pi pi-check" dropdownIcon="pi pi-cog" [model]="opcionesHV" severity="secondary"></p-splitbutton>'
);
